...
export const createSubDepartmentSchema = z.object({
  body: z.object({
    code: z.string().min(2, 'Sub-department code is required').trim().toUpperCase(),
    name: z.string().min(2, 'Sub-department name is required').trim(),
...
export const getSubDepartmentsQuerySchema = z.object({
  query: z.object({
...
export const updateSubDepartmentSchema = z.object({
  ...
  body: z
    .object({
      code: z.string().min(2, 'Sub-department code is required').trim().toUpperCase().optional(),
      name: z.string().min(2, 'Sub-department name is required').trim().optional(),
      cityId: mongoObjectId.optional(),
      plantId: mongoObjectId.optional(),
      departmentId: mongoObjectId.optional()
    })
